...
import com.baomidou.mybatisx.plugin.components.FlowPane;
import com.baomidou.mybatisx.plugin.components.GridPane;
...
public class CodeGeneratorPane extends VBox {
  ...
  ListTableModel<ModuleInfoGo> model = new ListTableModel<>(
    new ModuleInfo("config name", 120, false),
    new ModuleInfo(MODULE_PATH, 420, true, true),
    new ModuleInfo("base path", 140, true),
    new ModuleInfo("package name", 160, true)
  );

  private CheckBox commentCheckBox;
  private CheckBox lombokCheckBox;
  private CheckBox actualColumnCheckBox;
  private CheckBox JSR310DateAPICheckBox;
  private CheckBox useActualColumnAnnotationInjectCheckBox;
  private CheckBox toStringHashCodeEqualsCheckBox;
  private GridPane templateExtraPanel;
  private GridPane templateExtraRadiosPanel;
  ...
  private final EnumComboBox<TemplateAnnotationType> cboxAnnotationType;

  public CodeGeneratorPane() {
    HBox hBox = new HBox();
    hBox.addChildren(new Label("Annotation Type"));
    hBox.addSpacing(10);
    hBox.addChildren(this.cboxAnnotationType = new EnumComboBox<>(TemplateAnnotationType.class));
    add(hBox);

    FlowPane flowPane = new FlowPane();
    commentCheckBox = new CheckBox("Comment");
    lombokCheckBox = new CheckBox("Lombok");
    actualColumnCheckBox = new CheckBox("Actual Column");
    JSR310DateAPICheckBox = new CheckBox("JSR310: Date API");
    useActualColumnAnnotationInjectCheckBox = new CheckBox("Actual Column Annotation");
    toStringHashCodeEqualsCheckBox = new CheckBox("toString/hashCode/equals");
    flowPane.setPreferredSize(new Dimension(-1, 60));
    flowPane.addComponents(commentCheckBox, lombokCheckBox, actualColumnCheckBox, JSR310DateAPICheckBox, useActualColumnAnnotationInjectCheckBox, toStringHashCodeEqualsCheckBox);
    SwingUtils.setTitledBorder(flowPane, "Options");
    add(flowPane);

    templateExtraPanel = new GridPane(2, 2);
    templateExtraRadiosPanel = new GridPane(2, 2);
    VBox vBox = new VBox();
    SwingUtils.setTitledBorder(vBox, "Templates");
    vBox.add(templateExtraRadiosPanel);
    vBox.add(templateExtraPanel);
  ...
  private void selectAnnotation(String annotationType) {
    TemplateAnnotationType templateAnnotationType = TemplateAnnotationType.find(annotationType);
    if (annotationType != null) {
      cboxAnnotationType.setSelectedItem(templateAnnotationType);
    }
  }
...
//        generateConfig.setNeedMapperAnnotation(mapperAnnotationCheckBox.isSelected());
    ...
    String annotationTypeName = cboxAnnotationType.getNameOfSelectedItem();
  ...
  private class ModuleInfo extends ColumnInfo<ModuleInfoGo, String> {

    private int width;
    private boolean editable;
    private boolean moduleEditor;

    public ModuleInfo(String name, int width, boolean editable) {
      super(name);
      this.width = width;
      this.editable = editable;
    }

    public ModuleInfo(String name, int width, boolean editable, boolean moduleEditor) {
      super(name);
      this.width = width;
      this.editable = editable;
